...
class H2GCN(nn.Module):
    ...
    def init_adj(self, x, edge_index):
...
class FAGCN(nn.Module):
    def __init__(
        ...
        for _ in range(num_layers):
            self.convs.append(FALayer(hidden_channels, dropout))
            self.bns.append(nn.BatchNorm1d(hidden_channels))
        self.bns.append(nn.BatchNorm1d(hidden_channels))

        self.t1 = nn.Linear(in_channels, hidden_channels)
        self.t2 = nn.Linear(hidden_channels, out_channels)
